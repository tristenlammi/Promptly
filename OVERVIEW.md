...
- **Collaboration roles**: project shares carry a `role` (`editor` default, or read-only `viewer`). Owner + editors can edit settings, pin/unpin files, and add chats; viewers get a read-only project page (server enforces via `require_project_write`; the detail payload's `access_role` drives the UI gating). The share dialog has a per-invite role dropdown. **Unpin guard**: the owner can unpin any file, but a collaborator can only unpin files they pinned themselves (`chat_project_files.pinned_by`).
- **Per-chat file opt-out**: every chat in a project sees all pinned files by default; the chat header's "Files" popover lets the chat owner exclude specific files for that conversation only (`conversation_excluded_project_files`). The send path drops excluded files from both the full-dump set and the retrieval candidates.
- **Auto-memory (opt-in)**: a per-project toggle (`auto_memory_enabled`, off by default). When on, after a chat in the project produces a reply, a debounced background job (re)generates a single pinned **"Project Memory.md"** from the most recently active chat — reusing the summariser + the auto-pin/index path. Distinct from the manual "Save summary to project".
...
All tables are created by Alembic (`backend/alembic/versions/`), currently at migration **0071**. Ownership is enforced by FK + row-level queries, not Postgres RLS.
...
| `chat_projects` + `chat_project_files` | Projects | `user_id`, `name`, `system_prompt`, `default_model_id`/`default_provider_id`, `auto_memory_enabled`, shared file bag; `chat_project_files` carries `pinned_by` + the RAG indexing lifecycle (`indexing_status`/`indexing_error`/`indexed_content_hash`/`indexed_at`) |
| `project_shares` | Project collaborators | `project_id`, `invitee_user_id`, `status`, `role` (`editor`/`viewer`) |
| `conversation_excluded_project_files` | Per-chat opt-out of project pinned files | `conversation_id`, `file_id` |
...
| `/api/chat/projects` | `chat/projects_router.py` | Chat projects CRUD + file bag + `/{id}/usage` rollup; shares carry a `role`. `/conversations/search` takes an optional `project_id` filter; `/conversations/{id}/project-files` (GET/PUT) drives per-chat file opt-out. |
