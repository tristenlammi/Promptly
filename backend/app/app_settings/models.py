...
class AppSettings(Base):
    ...
    # Org-wide kill switch. Off by default — the admin must explicitly enable
    # the feature after configuring OAuth credentials. Overrides every per-user
    # email_mode setting so the admin can kill the feature instantly.
    email_integration_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    # Admin-provisioned Google Cloud project credentials (the app is self-hosted
    # so each deployment maintains its own OAuth client). Client secret is
    # Fernet-encrypted at rest (same pattern as smtp_password_encrypted).
    google_oauth_client_id: Mapped[str | None] = mapped_column(String(256), nullable=True)
    google_oauth_client_secret_enc: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Model used for AI email triage (category/priority/summary). Defaults to
    # a local model when configured so mail never leaves the box by default.
    # Both fields nullable: NULL = triage LLM disabled (heuristic pre-filter only).
    email_triage_provider_id: Mapped[uuid.UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("model_providers.id", ondelete="SET NULL"),
        nullable=True,
    )
    email_triage_model_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    # Daily token cap for the triage pipeline. NULL = no cap (use with care).
    email_triage_daily_token_cap: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
    ...
    @property
    def email_triage_configured(self) -> bool:
        """True when the admin has designated a triage model."""
        return bool(self.email_triage_provider_id and self.email_triage_model_id)

    @property
    def google_oauth_configured(self) -> bool:
        """True when Google OAuth credentials are present."""
        return bool(self.google_oauth_client_id and self.google_oauth_client_secret_enc)
