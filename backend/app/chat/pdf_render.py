...
# ---------------------------------------------------------------------
# SSRF / LFI hardening for the xhtml2pdf render (SECURITY)
# ---------------------------------------------------------------------
# xhtml2pdf resolves ``<img src>`` and CSS ``url()`` at render time with NO
# SSRF guard: an ``http(s)://`` src triggers an unfiltered server-side GET
# (following redirects — a blind SSRF into the docker network / cloud
# metadata), and a ``file://``/absolute path reads local disk (LFI). Both
# the model-authored markdown path and the user-authored document path feed
# HTML here, so we (1) strip every non-``data:`` resource before rendering,
# and (2) hand pisa a link_callback that refuses to resolve anything as a
# belt-and-braces backstop. ``data:`` URIs are decoded inline by pisa and
# never reach the callback, so inline/pasted images still work.
_IMG_SRC_DQ_RE = re.compile(
    r'<img\b[^>]*?\bsrc\s*=\s*"(?!\s*data:)[^"]*"[^>]*>', re.IGNORECASE
)
_IMG_SRC_SQ_RE = re.compile(
    r"<img\b[^>]*?\bsrc\s*=\s*'(?!\s*data:)[^']*'[^>]*>", re.IGNORECASE
)
_CSS_URL_RE = re.compile(
    r"url\(\s*['\"]?(?!\s*data:)[^)]*\)", re.IGNORECASE
)


def strip_external_pdf_resources(html: str) -> str:
    """Remove img/CSS resources that would make xhtml2pdf fetch a remote URL
    or read a local file. Only ``data:`` URIs (decoded inline, no I/O)
    survive — this is what closes the SSRF/LFI hole in PDF export."""
    if not html:
        return html
    html = _IMG_SRC_DQ_RE.sub("", html)
    html = _IMG_SRC_SQ_RE.sub("", html)
    html = _CSS_URL_RE.sub("url()", html)
    return html


def block_external_pdf_resource(uri, rel):  # noqa: ANN001 - pisa callback shape
    """xhtml2pdf link_callback that refuses every external/local resource.

    ``data:`` URIs are handled inline by pisa and never reach here, so any
    URI that does is a remote/file fetch we must not perform. Raising fails
    that single resource closed rather than performing the fetch."""
    raise OSError(f"blocked external resource in PDF render: {str(uri)[:80]}")
...
def render_markdown_to_pdf(
    ...
    # SECURITY: strip anything that would make xhtml2pdf reach out to a
    # URL/file at render time (SSRF/LFI); only inline data: images survive.
    full_html = strip_external_pdf_resources(full_html)
    ...
    result = pisa.CreatePDF(
        ...
        dest=out,
        encoding="utf-8",
        link_callback=block_external_pdf_resource,
    )
    if result.err:
