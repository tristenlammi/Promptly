...
class CodeInterpreterTool(Tool):
    ...
    max_per_turn = 6
    description = (
        "Execute Python code in a secure, sandboxed environment and get "
        "back stdout, errors, and any files it produces. Use this to "
        "actually RUN code rather than just describing what it would do: "
        "analyse data, do calculations, parse/transform files, and create "
        "charts. pandas, numpy, matplotlib, and openpyxl are available. "
        "The sandbox has NO internet access and is reset after every run. "
        "Files the user attached to this message (CSV, Excel, JSON, etc.) "
        "are already present in the working directory under their original "
        "filename — just open them by name (e.g. pd.read_csv('data.csv')). "
        "To return a chart, save it with matplotlib (e.g. "
        "plt.savefig('chart.png')) — saved image files are shown to the "
        "user automatically. Use print() for any results you want to see. "
        "Call this whenever the user asks you to compute, analyse a "
        "dataset, make a plot/graph from data, or check what some code "
        "actually outputs."
    )
    prompt_hint = (
        "Run Python in a secure sandbox (pandas/numpy/matplotlib "
        "available, no internet). Use it to actually compute results, "
        "analyse uploaded data files (they're in the working directory "
        "by their original filename), and produce charts — save plots "
        "with plt.savefig('name.png') and they'll be attached to your "
        "reply. print() anything you want to read back. Prefer running "
        "code over hand-waving an answer for data/maths questions."
    )
