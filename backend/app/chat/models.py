...
class Chart(UUIDPKMixin, TimestampMixin, Base):
    """A chart / graph — the backing entity for a ``kind='chart'`` workspace
    item. Mirrors :class:`Roster`: the whole chart (type + data rows + column
    config) lives in ``data`` (JSONB), saved single-user via a debounced PUT.

    ``data`` shape: ``{chartType:"bar"|"line"|"area"|"pie", labelKey, valueKeys:
    [..], rows:[{<col>: value}], sourceSheetId?}``. Manual data works with no
    other item; ``sourceSheetId`` remembers an optional Sheet the rows were
    imported (snapshotted) from.

    ``content_text`` holds a flattened text table of the data so a chat can
    answer questions about the numbers via workspace RAG; ``text_file_id`` is
    the backing Drive text file that carries it into ``knowledge_chunks``.
    """
    ...
    def __repr__(self) -> str:
        return f"<Chart id={self.id} title={self.title!r}>"


class DataView(UUIDPKMixin, TimestampMixin, Base):
    """A read-only database view — the backing entity for a ``kind='dataview'``
    workspace item. References an admin-configured :class:`DataSource` and a
    ``SELECT`` query; the last run's result is cached in ``data`` (JSONB:
    ``{columns, rows, truncated, row_count}``) so the item is viewable and its
    ``content_text`` feeds workspace RAG without hitting the DB every time.
    """
    ...
    title: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
        default="Untitled data view",
        server_default="Untitled data view",
    )
    # The data source this queries. SET NULL so deleting a source doesn't
    # cascade away the item (it just can't run until re-pointed).
    data_source_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("data_sources.id", ondelete="SET NULL"), nullable=True
    )
    sql: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Cached last result: {columns, rows, truncated, row_count}.
    data: Mapped[Any | None] = mapped_column(JSONB, nullable=True)
    content_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    text_file_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("files.id", ondelete="SET NULL"), nullable=True
    ...
    def __repr__(self) -> str:
        return f"<DataView id={self.id} title={self.title!r}>"
