...
export interface UserSettings {
   ...
   *  ``"workspaces" | "tasks"``. Absent / ``[]`` = show all. */
  hidden_nav?: string[];
  /** Opt-in nav surfaces the user has explicitly enabled. These ship hidden
   *  by default and appear once listed here.
  ...
  /** Memory behaviour: ``"off"`` (none), ``"auto"`` (capture + inject),
   *  or ``"manual"`` (inject saved facts, user adds them — no auto
   *  capture). Absent = derive from ``memory_enabled`` (on → auto). */
